...
The UI loop field allows designers to place table/loop placeholders on the canvas by drawing a rectangle after clicking "Add Loop" in the toolbar. Once placed, the right panel exposes all loop configuration: JSON key (auto-prefixed with `loops.`), max rows, max columns, multi-page toggle with max pages input, column definitions with drag-to-reorder, and granular style controls for headers, rows, and cells. Column definitions include per-column style overrides that take precedence over the global row style. The loop field is the most complex field type, mapping directly to the table rendering logic in the core library.

## Requirements

- [ ] REQ-001: Clicking "Add Loop" in the toolbar enters a draw mode where the user draws a rectangle on the canvas by clicking and dragging
- [ ] REQ-002: After drawing, a new loop field is created at the drawn position with the drawn width and height
- [ ] REQ-003: The right panel displays all editable properties for the selected loop field
- [ ] REQ-004: JSON key input is auto-prefixed with `loops.` -- the user enters only the suffix (e.g. typing "marks" produces `loops.marks`)
...
1. User clicks "Add Loop" in the toolbar. Cursor changes to a crosshair indicating draw mode.
2. User clicks and drags on the canvas to draw a rectangle. On mouse release, the loop field is created.
3. The field appears on the canvas as a table outline with a header row and placeholder data rows.
4. The right panel populates with loop field properties, pre-filled with defaults: maxRows 10, maxColumns 3, multiPage false, empty columns list.
5. User edits the JSON key suffix to "marks". The field's `jsonKey` becomes `loops.marks`.
...
- Duplicate column keys within the same loop: show a validation error
...
- Attempting to add a loop field before a background image is uploaded: blocked with a message (dependent on spec 009 canvas requirements)
...
```ts
interface LoopFieldDefinition {
  id: string
  type: 'loop'
  groupId: string | null
  required: boolean
  jsonKey: string // e.g. "loops.marks"
  placeholder: null
  x: number
  y: number
  width: number
...
```ts
interface LoopFieldPropsPanel {
  field: LoopFieldDefinition
  groups: Group[]
  onUpdate(updates: Partial<LoopFieldDefinition>): void
  onStyleUpdate(updates: Partial<LoopFieldStyle>): void
  onAddColumn(): void
...
- [ ] AC-001: Clicking "Add Loop" enters draw mode; drawing a rectangle on the canvas creates a loop field at that position with the drawn dimensions
- [ ] AC-002: The right panel displays all loop field properties when a loop field is selected
- [ ] AC-003: JSON key is auto-prefixed with `loops.` -- entering "marks" produces `jsonKey: "loops.marks"`
...
- [ ] AC-019: A loop field with zero columns defined is valid but shows an empty box on the canvas with a "No columns defined" message

## Dependencies

- Spec 002 — Template Schema (loop field style schema, column definition structure)
- Spec 005 — Loop/Table Rendering (table rendering rules, multi-page behaviour, cell styling)
- Spec 009 — UI Canvas (canvas infrastructure, selection, resize handles, draw mode)

## Notes

- The right panel component for loop fields lives at `packages/ui/src/components/RightPanel/LoopFieldProps.tsx`.
...
- The multi-page max pages setting on a loop field updates `meta.maxPages` in the manifest. If multiple loop fields have different max pages values, the highest value should be used. This interaction needs careful handling.
