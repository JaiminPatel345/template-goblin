...
The `generatePDF` function is the hot path of the library. It takes a pre-loaded `LoadedTemplate` (from `loadTemplate`) and an `InputJSON` data object, validates the input data, registers fonts, renders the background image, and renders all fields in z-index order to produce a PDF as a `Buffer`. It performs zero disk I/O -- everything operates from in-memory data. A convenience function `generatePDFFromFile` is also provided that combines `loadTemplate` and `generatePDF` into a single call for one-off usage.
...
- [ ] REQ-009: Loop/table fields are rendered according to the loop rendering rules defined in spec 005
- [ ] REQ-010: Multi-page behaviour follows the rules defined in spec 006 -- only loop fields with `multiPage: true` can trigger additional pages
- [ ] REQ-011: The default output is a single page. Additional pages are created only when loop data overflows
- [ ] REQ-012: When a new page is added, the background image is re-rendered on that page before any table continuation
- [ ] REQ-013: Non-loop fields (text, image) are rendered only on page 1
...
7. Each field is rendered in order:
   ...
   - Loop fields: rendered as tables per spec 005, potentially spanning multiple pages per spec 006
...
- Empty array for a loop field: table header may or may not render (open question); no data rows drawn
- Image field value as base64 string: decoded to Buffer before passing to PDFKit
- Image field value as Buffer: used directly
- Loop field with exactly the number of rows that fit on one page: no second page created
- Loop field with `multiPage: false` and overflowing data: rows are truncated to fit the bounding rectangle on page 1
- Multiple loop fields on the same template: each loop is independent; only loops with `multiPage: true` trigger new pages
...
- `INVALID_DATA_TYPE`: A field receives data of the wrong type (e.g. a string where an array is expected for a loop, or a non-image value for an image field). Error includes the field `jsonKey` and expected vs actual type.
- `MAX_PAGES_EXCEEDED`: A multi-page loop would require more pages than `meta.maxPages` allows. Error includes the number of pages required and the maximum allowed.
...
- [ ] AC-004: `generatePDF` with a missing required loop field rejects with `MISSING_REQUIRED_FIELD`
- [ ] AC-005: Fields are rendered in ascending z-index order -- a field with `zIndex: 2` renders on top of a field with `zIndex: 1`
- [ ] AC-006: Background image is rendered on every page (including pages added by multi-page loops)
...
- [ ] AC-009: Loop fields render with correct header, row styles, column widths, cell padding, and borders
- [ ] AC-010: A multi-page loop that overflows creates additional pages, each with the background image and continued table (with header re-rendered)
- [ ] AC-011: A multi-page loop that would exceed `meta.maxPages` rejects with `MAX_PAGES_EXCEEDED`
- [ ] AC-012: Non-loop fields appear only on page 1 and are not duplicated on subsequent pages
...
- Open question: when a loop field has an empty array in the input data, should the table header still be rendered? Current leaning: no -- skip the entire field if the array is empty.
