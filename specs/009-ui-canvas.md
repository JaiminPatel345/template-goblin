...
- [ ] REQ-036: Canvas scroll symmetry — at any zoom level, the scrollable overflow around the Stage is symmetric on both axes. Scrolling fully to the start (left or top) reveals the Stage's corresponding edge flush with the viewport's start edge, mirroring the end-edge behaviour. Implementation note: the scroll container uses `display: flex; justify-content: safe center; align-items: safe center` so flex centering degrades to start-alignment when the Stage overflows (avoiding the long-standing "left-side unreachable" bug of plain `justify-content: center` with `overflow: auto`).
...
- [ ] REQ-037: Zoom binding — `Ctrl + wheel` (or `Cmd + wheel` on macOS) zooms the canvas. The wheel's `deltaY` controls direction; `deltaY < 0` increases zoom, `deltaY > 0` decreases. Trackpad pinch-zoom emits synthetic `wheel` events with `ctrlKey: true` and is handled by the same code path. Zoom is clamped to `[0.1, 5]`.
- [ ] REQ-038: Zoom-at-cursor — zoom operations anchor on the pointer position, not the canvas center. After updating `zoom`, the scroll container's `scrollLeft`/`scrollTop` are adjusted so the canvas point that was under the cursor before the zoom remains under the cursor after. Formula: `scrollX = (scrollX + cx) * (newZoom / oldZoom) - cx`, where `cx` is the cursor x relative to the container (`e.clientX - rect.left`); same for y.
- [ ] REQ-039: Scroll binding — plain `wheel` (no modifier) scrolls the canvas vertically. `Shift + wheel` scrolls horizontally. Neither action changes zoom. This is implemented in the same wheel handler as REQ-037 by branching on modifier keys; default browser behaviour is prevented throughout so the page itself does not scroll when the cursor is over the canvas.
- [ ] REQ-040: Wheel listener attachment — the wheel handler MUST survive container remounts. The container mounts under different React trees in the onboarding state vs the canvas state; a bare `useEffect([])` pattern attaches to the first container and never re-binds when the canvas container mounts, resulting in dead zoom. Implementation MUST use a ref-callback (`ref={setContainerRef}`) that detaches the listener from the old node and attaches it to the new node on every assignment.
...
- [ ] AC-036: At any zoom level the canvas scrollable overflow is symmetric: scrolling fully to the left shows the Stage's left edge flush with the viewport, mirroring right-edge behaviour. Regression reference: `packages/ui/src/utils/scrollBounds.ts` + its unit test pin the invariant `|overflowStart - overflowEnd| < ε` for a range of zoom levels.
